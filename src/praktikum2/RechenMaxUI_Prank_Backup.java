...
public class RechenMaxUI_Prank_Backup extends JComponent {
    ...
    public void actionPerformed(final ActionEvent e) {
      ...
      if (getResultText().length() < 17) {
        ...
        if ("0".equals(resultText)) {
          if (getResultText().startsWith("-")) {
            addResultText(num);
          } else {
            setResultText(num);
          }
        } else {
          if (resultText.startsWith("-0") && !resultText.contains(",")) {
            setResultText("-" + resultText.substring(1, 1) + num);
          } else {
  ...
  // create Buttons
  public RechenMaxUI_Prank_Backup() {
    initialize();
  }
